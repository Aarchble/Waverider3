...
public class Combustion : Process
{
    ...
    public Mesh GetCombustionMesh(NearStream nearStream, bool _debug = false)
    {
        if (nearStream is InternalStream)
        {
            // InternalStream
            Mesh mesh = new();
            mesh.vertices = new Vector3[4] { nearStream.Inlet[0], nearStream.Inlet[1], nearStream.Outlet[1], nearStream.Outlet[0] };
            mesh.uv = new Vector2[4] { new Vector2(0f, 0f), new Vector2(0f, 1f), new Vector2(0f, 1f), new Vector2(0f, 0f) };
    ...
    public Mesh GetCombustionMesh(NearStream nearStream, Vector3 inletShockStart, Vector3 inletShockEnd, bool _debug = false)
    {
        if (nearStream is InternalStream)
        {
            // InternalStream
            ...
            mesh.vertices = new Vector3[4] { inletShockStart, inletShockEnd, nearStream.Outlet[1], nearStream.Outlet[0] };
            mesh.uv = new Vector2[4] { new Vector2(0f, 0f), new Vector2(0f, 1f), new Vector2(0f, 1f), new Vector2(0f, 0f) };
